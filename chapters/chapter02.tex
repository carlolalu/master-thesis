...
\\
&= \sum_{g=1}^{\infty} 
	\int d\Prb(\btime^{p,g} = s) \Prb (\hmactive_{\btime^{p,g}} = m \big| \btime^{p,g} = s)
\\
&= \sum_{g=1}^{\infty} 
	\underbrace{\int d\Prb(\btime^{p,g} = s)}_{:=\Zhi(p,g,s) ds} \Prb (\hmactive_s = m \big)
\\
&= \sum_{g=1}^{\infty} 
	\Zhi(p,g,s) ds \Pois_{Ns\lambda}(\{m\})
\end{aligned}
\end{equation}

And since $\Zhi(p,g,s)=d\Gma_{g,\lambda}$, it is independent from $p$, we have that $\Prb (\Parentof'(m) = p) = const(m)$, \ie it is independent from $p$ as well, and thus uniformly distirbuted on $\{1,\ldots,N\}$.

The second question is translatable to

\begin{equation*}
\invquest 
	\quad \forall p,g . 
		\quad \Prb (\Parentof'(m) = p,\Parentof'(m+1) = q) =^{?} \Prb (\Parentof'(m) = p)\Prb (\Parentof'(m+1) = q) = \frac{1}{N^2}
?
\end{equation*}
...
= \Prb(\exists g,h \in \N.\quad \hmactive^p_{\btime_m} = g, \hmactive^q_{\btime_{m+1}} = h)
\\
& = \Prb(\exists g \in \N.\quad \hmactive^p_{\btime_m} = g, \hmactive^q_{\btime_m,\btime_{m+1}} = h)
\\
&= \sum_{g=1}^{\infty} \Prb(\hmactive^p_{\btime_m} = g, \hmactive^q_{\btime_m,\btime_{m+1}} = h)
\\
&= \sum_{g,h=1}^{\infty} \int d\Prb(\btime_m = s, \btime_{m+1} = t)  \Prb(\hmactive^p_{\btime_m} = g,
	\hmactive^q_{\btime_{m+1}} = h \big| \btime_m = s, \btime_{m+1} = t)
\\
& \text{(The integrand is positive, thus $\sum$ can enter the integral)}
\\
&= \int d\Prb(\btime_m = s, \btime_{m+1} = t)  \underbrace{\sum_{g,h=1}^{\infty} \Prb(\hmactive^p_{\btime_{m}} = g,\hmactive^q_{\btime_{m+1}} = h \big| \btime_m = s, \btime_{m+1} = t)}_{\Ch(p,q,s,t)}
\end{aligned}
\end{equation}
...
\begin{aligned}
\Ch(p,p,s,t) 
&= \sum_{g,h=1}^{\infty} \Prb(\hmactive^p_{\btime_{m}} = g,\hmactive^p_{\btime_{m+1}} = h \big| \btime_m = s, \btime_{m+1} = t)\1_{h=g+1}
\\
&= \sum_{g=1}^{\infty} \Prb(\hmactive^p_{\btime_{m}} = g,\hmactive^p_{\btime_m,\btime_{m+1}} = 1, 
	\sum_{k\neq p} \hmactive_{\btime_m,\btime_{m+1}}^k = 0 \big| \btime_m = s, \btime_{m+1} = t)
\\
&= \sum_{g=1}^{\infty} \Prb(\hmactive^p_s = g) \Prb(\hmactive^p_{s,t} = g+1 ) \Prb(\sum_{k\neq p} \hmactive_{s,t}^k = 0)
\\
&= \sum_{g=1}^{\infty} \Pois_{s\lambda}(\{g\}) \Pois_{(t-s)\lambda}(\{g+1\}) \Pois_{(N-1)(t-s)\lambda}(\{0\})
\end{aligned}
\end{equation}
...
\begin{aligned}
\Ch(p,p,s,t) 
&= \sum_{g,h=1}^{\infty} \Prb(\hmactive^p_{\btime_{m}} = g,\hmactive^q_{\btime_{m+1}} = h \big| \btime_m = s, \btime_{m+1} = t)\1_{h=g+1}
\\
&= \sum_{g}^{\infty} \Prb(\hmactive^p_{\btime_{m}} = g, \hmactive^q_{\btime_{m},\btime_{m+1}} = 1 \big| \btime_m = s, \btime_{m+1} = t)\1_{h=g+1}
\end{aligned}
\end{equation}
