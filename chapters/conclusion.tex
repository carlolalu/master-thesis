...
\subsection{Comparison with the original paper of Ana Cañizares García}

The original paper who inspired this master thesis (\cite{ana}) modelled the movement of amoebas as particles attracting each others with a Coulomb-like potential, to find the famous Keller-Segel equation,

[[add here up quote to the original Keller-Segel]]

in particular of the specific case in which the chemoattractant diffuses much faster than the organisms releasing it:
...
\begin{equation}
		\partial_t \adens = - \chi \nabla ( (\attraction_C \star \adens) \adens) + \bigtriangleup \adens
\end{equation}

where $\attraction_C:= \frac{x}{2\pi \abs{x}^2}$ is the previously mentioned Coulomb-like attraction.
This thesis models some different scenario: the function $\attraction$ taken into consideration is Gaussian-like, and the diffusion coefficient $\diffusion_\camp$ of the cAMP molecules is finite (\ie of comparable order of magnitude as $\diffusion_\amba$, the equivalent of the amoebas).
What happens to the effective equations we have found, in case $\diffusion_\camp >> \diffusion_\amba$?
