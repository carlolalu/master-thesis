...
\begin{equation*}
	\sup_{t \in [0,T]} \const_{t,y} = \sup \const_{T,y}
\end{equation*}
...
\begin{equation*}
	\logQuest{
		\sup_{q \leq Q} \const_{q} \leq \const_Q 
	}
\end{equation*}

also in this case it is a good solution to show that $f(q) := \const_q$ is an increasing function.
...
will mean a function with the above characteristics in $t$, and furthermore dependent on some mathematical objects $a,b,c$, while

\begin{equation*}
	\Fup(t,q)
\end{equation*}

will be an increasing function bounded in both arguments.
