...
and consider some kind of norm

\begin{equation*}
	\Norm{f(t,x)}
\end{equation*}

We adopt the subsequent convention:

\begin{equation*}
	\left\{
	\begin{aligned}
		\norm{f(t,x)}_\infty &:= \max_{1 \leq i \leq n} \norm{f_i(t,x)}		&& \text{where $x,t$ are fixed}
		\\
		\Norm{f(t,x)}_\infty &:=  \max_{1 \leq i \leq n} \Norm{f_i(t,\cdot)}_{\Lpspace^\infty(\R^m)}			&& \text{where $t$ is fixed}
	\end{aligned}
	\right.
\end{equation*}
...
\begin{equation*}
	\sup_{s_1 \leq t \leq s_2} \Norm{f(t,x)}_\infty = \sup_{s_1 \leq t \leq s_2} \max_{1 \leq i \leq n} \Norm{f_i(t,\cdot)}_{\Lpspace^\infty(\R^m)}
\end{equation*}



\subsection{Generic positive constants and positive increasing functions}
